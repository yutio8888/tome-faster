...
--
-- Isolated measurement copy: start{on_phase=function(name) ... end} optionally
-- receives these markers inline from step(), once each on a successful scene:
--   entry_call_begin  immediately before stock forceUseTalent(Fearscape).
--   entry_call_return immediately after that call returns, before assertions.
--   entry_complete    first step observing the stock plane/capture/death-wrapper
--                     identities and an empty tick-end queue, before token setup
--                     or the entered actor/map snapshots.
--   exit_call_begin   immediately before stock die(nil), or forceUseTalent when
--                     mode="active"; this is the synchronous exit trigger.
--   exit_call_return  immediately after that call returns, before assertions or
--                     the exit-requested actor/callback snapshots.
--   exit_complete     first step observing the original source level/zone,
--                     restored original on_die identities, inactive sustain and
--                     an empty tick-end queue. Emitted before entity/item checks,
--                     the full source-map token scan, or inspect()/snapshots.
-- begin -> call_return measures the synchronous trigger. begin -> complete also
-- includes asynchronous original callbacks and CPU until the first observing
-- step (frame/poll scheduling, the frozen helper's small exit_requested snapshot,
-- and readiness checks). A complete marker is
-- NOT a timestamp injected at the original callback's last instruction. Accept
-- a measured scene only after step() finally returns report.ok == true.
-- The callback must only capture cheap timing samples: no yield, log/inspect,
-- RNG/game mutation or further game calls. An exception fails the diagnostic
-- through its existing xpcall and can leave an incomplete marker sequence.
-- No talent, actor death method, or scheduler method is wrapped/replaced; no
-- extra tick-end callback, gameplay turn, or Astar reference is introduced.
local M = {}
...
local function actor(entity)
    ...
    local safe_grid = entity.ai_state and entity.ai_state.safe_grid
    row.astar_map_present = safe_grid and safe_grid.Astar and safe_grid.Astar.map and true or false
    return row
end
...
function M.start(options)
    ...
    assert(options.on_phase == nil or type(options.on_phase) == "function", "on_phase must be a function or nil")
    local on_phase = options.on_phase
    ...
    local function prepare()
        ...
        assertPaused()
        if on_phase then on_phase("entry_call_begin") end
        local ret = caster:forceUseTalent(caster.T_DEMON_PLANE, {ignore_energy=true, ignore_cd=true})
        if on_phase then on_phase("entry_call_return") end
    ...
    local function entered()
        ...
        if on_phase then on_phase("entry_complete") end
        ...
        if mode == "casterdead" then
            ...
            if on_phase then on_phase("exit_call_begin") end
            local ret = caster:die(nil)
            if on_phase then on_phase("exit_call_return") end
            assert(ret and caster.dead, "stock NPC death did not complete")
        else
            if on_phase then on_phase("exit_call_begin") end
            local ret = caster:forceUseTalent(caster.T_DEMON_PLANE, {ignore_energy=true})
            if on_phase then on_phase("exit_call_return") end
            assert(ret, "stock forceUseTalent refused deactivation")
        end
    ...
    local function exited()
        ...
        if on_phase then on_phase("exit_complete") end
