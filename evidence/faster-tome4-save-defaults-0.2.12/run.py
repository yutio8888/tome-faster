#!/usr/bin/env python3
"""Verify 0.2.12 defaults and readback on fresh, disposable homes."""
import argparse
import fcntl
import hashlib
import json
...
HOME_ROOT = WORKSPACE / 'test-saves/save-defaults-20260914'
...
EXPECTED_ADDON = '7e2c7a8d8aac9aa9fec26e5d2686c1871d2878d99df74303af5e97fac8dfe7a5'
CASES = {
    'normal': None,
...
def run(name, display, sequence=None, plan_sha=None):
    ...
    if mode == 'save' and variant == 'candidate':
        # All three feature keys are absent: exercise the package defaults.
        options.update(compact_inventory=True, fearscape_cleanup=True)
    else:
        # Independent explicit opt-outs also select the original reader on reload.
        production.update(OFF)
    if mode != 'save':
    ...
    with (ROOT / ('xvfb-' + args.names[0] + '.log')).open('w') as log:
        ...
        try:
            ...
            for index, name in enumerate(args.names, 1):
                run(name, args.display, None, plan_sha)
        finally:
