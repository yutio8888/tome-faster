...
def run(name, display):
    ...
    reload_name = name[len('reload-'):] if name.startswith('reload-') else None
    home_root = WORKSPACE / 'test-saves/save-size-20260914'
    home = home_root / name / 'home'
    home.mkdir(parents=True, exist_ok=False)
    base = home / '.t-engine/4.0'
    assert digest(SAVE) == EXPECTED_SAVE
    if reload_name:
        shutil.copytree(home_root / reload_name / 'home/.t-engine/4.0/tome/save',base / 'tome/save')
    else:
...
save_size_probe = {compact=%s,reload=%s}
''' % ('true' if compact else 'false','true' if reload_name else 'false'))
    if reload_name:
        with (settings / 'save-size-probe.cfg').open('a') as config:
            config.write('faster_tome.load_queue = false\n')
    engine = ROOT / 'runtime/t-engine'
    ...
    files = [Path(__file__)] + sorted((ROOT / 'runtime/game/addons/tome-faster').rglob('*.lua'))
    archive_addon = ROOT / 'runtime/game/addons/tome-faster.teaa'
    if archive_addon.is_file():
        files.append(archive_addon)
    files += sorted((ROOT / 'runtime/game/addons/tome-save-size-probe').rglob('*.lua'))
    meta = dict(command=command, engine_sha256=digest(engine), save_sha256=digest(SAVE),
                ...
                compact=compact,reload_from=reload_name)
    (session / 'input.json').write_text(json.dumps(meta, indent=2) + '\n')
    print('Starting ' + name, flush=True)
    started = time.monotonic()
    timed_out = False
    with (session / 'game.log').open('w') as log:
        process = subprocess.Popen(command, cwd=ROOT / 'runtime', env=env, stdout=log, stderr=subprocess.STDOUT)
        ...
        if path.is_file() and path.suffix in ('.teag','.teaz','.teaw','.teal'):
            with zipfile.ZipFile(path) as archive:
                ...
                result['archives'].append(dict(name=str(path.relative_to(base / 'tome/save')),bytes=path.stat().st_size,
                    ...
                    compact_names=all(i.filename=='main' or i.filename.isdigit() for i in infos)))
