...
function M.run(realgame)
    ...
    local installed = Player.saveUUID
    -- Count the delegate actually used by the installed method. With gzip
    -- enabled this is FasterGzip's exporter, not the untouched engine method.
    local exporter = upvalue(installed, "delegate") or upvalue(installed, "original") or installed
    local exporterInfo = debug.getinfo(exporter, "S")
    assert((exporterInfo.source == "@/engine/interface/PlayerDumpJSON.lua"
        and exporterInfo.linedefined == 41 and exporterInfo.lastlinedefined == 90)
        or exporterInfo.source:find("FasterGzip.lua", 1, true), "unknown installed character exporter")
    assert(realgame.player.saveUUID == installed, "player does not use the installed exporter")
    local exportenv = getfenv(exporter)
    ...
    local ok, err = pcall(function()
        ...
        setfenv(exporter, setmetatable({json = countedJSON}, {__index = exportenv}))
        ...
        for i, name in ipairs{"original_party_export", "without_party_export"} do
            ...
            assert(snapshot.player.saveUUID == installed, "snapshot lost installed exporter")
    ...
    local summary = {online_equal = true, captured_online_orders = 2, real_network_calls = 0,
        ...
        installed_exporter_source = exporterInfo.source}
