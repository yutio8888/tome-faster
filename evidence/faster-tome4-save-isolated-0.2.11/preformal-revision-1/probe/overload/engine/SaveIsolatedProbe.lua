...
function M.start()
    ...
    ffi.cdef[[struct isolated_save_ts {long sec;long nsec;}; int clock_gettime(int,struct isolated_save_ts*);]]
    local ts=ffi.new('struct isolated_save_ts[1]')
    ...
    local options={compact_inventory=ft.compact_inventory==true,fearscape_cleanup=ft.fearscape_cleanup==true,
    ...
    local function inventoryFixture()
        ...
        for i=1,count do assert(player:addObject(inventory,objects[i],true)) end
    ...
    Game.display=function(self,...)
        ...
        if phase=='ready' then
            ...
            self:onTickEnd(function()
                ...
                local before=state()
                if cfg.reload then
                    assert(debug.getinfo(Savefile.loadReal,'S').source=='@/engine/Savefile.lua','original reader required')
