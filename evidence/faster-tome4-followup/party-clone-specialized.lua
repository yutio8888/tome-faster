...
local unused = [[	if use_saveinstead and (d.__ATOMIC or d.__CLASSNAME) and d.__SAVEINSTEAD then
		d = d.__SAVEINSTEAD
		if clonetable[d] then return d, 1 end
	end
]]
candidate = replace(candidate, unused, "")
candidate = candidate:gsub(", noclonecall, use_saveinstead", ""):gsub(", nil, nil", "")
candidate = candidate:gsub("not noclonecall and ", "")
