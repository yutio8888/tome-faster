...
def run(name, display):
    ...
    bare = name.startswith('bare-')
    compact = True
    reload_name = name[len('reload-'):] if name.startswith('reload-') else name[len('bare-'):] if bare else None
    source_name = reload_name or (name[len('resave-'):] if name.startswith('resave-') else None)
    home_root = WORKSPACE / 'test-saves/save-compaction-20260914'
    ...
    if bare:
        content = content.replace("'faster', ", "")
...
''' % ('true' if candidate else 'false','true' if reload_name else 'false','true' if bare else 'false','true' if 'inventory' in name else 'false'))
    if reload_name:
    ...
    runtime = ROOT / ('runtime-no-faster' if bare else 'runtime')
    if not bare:
    ...
    meta = dict(measurement_sequence=sequence, acceptance_plan_sha256=plan_sha, started_unix_ns=time.time_ns(), command=command, engine_sha256=digest(engine), save_sha256=digest(SAVE),
                ...
                compact=compact,candidate=candidate,bare=bare,reload_from=reload_name,source_session=source_name,
