...
function M.start()
    ...
    elseif cfg.candidate then
        assert(not package.loaded['engine.FasterInventory'] and not package.loaded['engine.FasterFearscape'], 'default loaded experiment module')
        assert(inventory_source=='@/mod/class/Player.lua' and fearscape_source=='@/data/talents/corruptions/shadowflame.lua', 'default changed event methods')
    end
    local sample_name_62
    if not cfg.bare then
        local sample=setmetatable({tables={}}, {__index=Savefile})
        for i=1,62 do sample_name_62=sample:getFileName({}) end
        assert(sample_name_62==(cfg.experimental and '10' or '62'), 'unexpected name encoding')
    end
    emit({kind='setup',sample_name_62=sample_name_62,experiments_enabled=cfg.experimental and true or false,inventory_source=inventory_source,fearscape_source=fearscape_source,compact=cfg.compact and true or false,candidate=cfg.candidate and true or false,
    ...
    local Scenario=require 'engine.FearscapeScenario'
    local scenario_step,scenario_report,scenario_poll_queued
    ...
    Game.display=function(self,...)
            ...
            self:onTickEnd(function()
                ...
                if done then
                    ...
                    if not report.ok then
                        emit({kind='error',report=report});phase='failed';core.game.exit_engine();return
                    end
                    scenario_report=report
                    emit({kind='fearscape_scenario',report=report})
                    phase='ready'
                end
                scenario_poll_queued=false
            end,'fearscape_probe_step')
            core.game.requestNextTick()
        end
        if phase=='warmup' and frames>=30 and clock(1)-started>=2000 then
            if cfg.fearscape and not cfg.reload then
                scenario_step=Scenario.start{mode='casterdead',seed=6140914,expect_cleanup=cfg.experimental}
                phase='scenario'
        ...
        if phase=='ready' then
            ...
            self:onTickEnd(function()
                ...
                if cfg.inventory and not cfg.reload then inventoryFixture(cfg.experimental) end
                local before=state()
                if cfg.reload then
                    local loader=debug.getinfo(Savefile.loadReal,'S')
                    assert(loader.source=='@/engine/Savefile.lua','reload must use original loadReal')
                    if cfg.fearscape then
                    local restored=Scenario.inspect(assert(cfg.expected))
