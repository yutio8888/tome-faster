...
function M.start()
 ...
 local function finish(reason)
  ...
  emit{kind='phase_end',phase=phase,reason=reason,wall_ms=now()-phase_wall,cpu_ms=cpu()-phase_cpu,
   ...
   records=records,frames_ms=frames,actions=actions,slow=slow,life=game.player.life,x=game.player.x,y=game.player.y}
 ...
 local function diagnostic(name)
  local ok,err=pcall(function() require(name).run(game) end)
  if not ok then
  ...
  if phase=='load' then
   ...
   elseif phase=='save' then
    if not queued and not save_requested then
     queued=true; self:onTickEnd(function() game:saveGame(); save_requested=true; queued=false end,'yron_stutter_save')
    end
