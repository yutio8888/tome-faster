...
| 版本 | 源码基线 | 说明 |
| --- | --- | --- |
| 0.2.12 | 本分支 0.2.12 工作树 | 库存归属压缩和 Fearscape 结束清理默认开启，base62 默认关闭；可分别配置关闭，见[默认配置](../docs/save-defaults.md) |
| 0.2.11 | 本分支 0.2.11 工作树 | 新物品归属数字 ID、Fearscape 结束引用清理与 base62 编号实验；全场景 CPU 门槛未获证明，三项均默认关闭，保留 0.2.10 十进制默认路径；见[验收报告](../docs/save-compaction.md) |
| 0.2.9 | 本分支 0.2.9 工作树 | A* 邻格与开放集合优化、FBO 保护和遮罩批处理兼容、聊天测量弱引用修复；真实地图寻路 CPU 测量见报告 |
| 0.2.10 | 本分支 0.2.10 工作树 | 保存内部对象使用短编号；保留原版读取器、压缩级别和保存流程，默认启用 |
| 0.2.8 | 本分支 0.2.8 功能提交 | 纯 Lua FBO 回收保护，支持配置关闭；Windows 与长期运行待验证 |
